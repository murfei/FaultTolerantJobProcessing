...
Ein Auftrag muss auch dann zuverlässig verarbeitet werden, wenn während seiner Verarbeitung ein Fehler auftritt.
Dabei darf der Job weder unbemerkt verloren gehen noch infolge eines Fehlers mehrfach wirksam werden.
...
Insbesondere in industrienahen und sicherheitskritischen Anwendungen müssen Hintergrundprozesse nicht nur funktional korrekt,
sondern auch zuverlässig, nachvollziehbar und fehlertolerant ausgeführt werden.
Aber auch in nicht-kritischen Systemen ist Zuverlässigkeit von großer Bedeutung, sowohl für die Nutzerfreundlichkeit der Anwendung
als auch zur Vermeidung von Produktivitäts- und Gewinneinbußen in Folge von Fehlern
...
Nachrichten können verloren gehen \parencite[Kap.~8]{kleppmann_designing_2017}.
...
\parencite{alquraan_analysis_2018}.
Für ein Job-Processing-System stellt sich damit nicht nur die Frage, ob ein Job unter normalen Bedingungen korrekt verarbeitet wird,
sondern auch, wie sich das System während und nach Störungen verhält.

\section{Problemstellung}
\label{sec:problemstellung}
Ohne geeignete Mechanismen führen Teilausfälle zu inkonsistenten Systemzuständen oder dem Verlorengehen von Nachrichten.
...
Nahezu jedes Backend-System, das über einfache Create-, Read-, Update- und Delete-Operationen hinausgeht, muss Aufgaben
zuverlässig im Hintergrund verarbeiten und dabei konsistente Datenbestände gewährleisten \parencite[Kap.~4]{richardson_microservices_2018}.
Fehler in diesem Bereich führen zu Datenverlust, inkonsistenten Geschäftszuständen sowie Bedarf an manuellen Eingriffen und
...
\parencite{microsoft_corporation_hrsg_best_2026-1} und müssen bereits beim Entwurf eines Systems berücksichtigt werden.
...
Für klein- und mittelgroße Systeme stellt dies einen verhältnismäßig hohen Betriebsaufwand dar.
...
Welche Architekturmechanismen und Entwurfsentscheidungen ermöglichen es, in einem Job-Processing-System
...
\begin{enumerate}[label=TF-\arabic*:, ref=TF-\arabic*]
    ...
    wiederaufgenommen werden, ohne dass Aufträge verloren gehen?
\end{enumerate}

\section{Zielsetzung}
\label{sec:zielsetzung}
Ziel dieser Arbeit ist der Entwurf, die prototypische Implementierung und die Evaluation
eines fehlertoleranten Job-Processing-Backends auf Basis einer Datenbank.
Die prototypische Implementierung erfolgt in Java in Kombination mit dem Spring-Framework.
...
\begin{enumerate}[label=TZ-\arabic*:, ref=TZ-\arabic*]
    ...
    funktionsfähiger Prototyp umgesetzt werden, der die definierten Zusicherungen unter realistischen Bedingungen einhält.
...
\begin{enumerate}[label=H-\arabic*:, ref=H-\arabic*]
    ...
    Fehlerinjektion reproduzierbar überprüfen und nachweisen. %TODO: schlechte Hypothese. Welches Ergebnis hätte sie widerlegt?
\end{enumerate}
...
Dazu zählt das Vorgehen bei der Literaturrecherche, der Anforderungserhebung sowie dem Systementwurf, als auch die Methodik
der späteren Evaluation.
...
\Cref{ch:diskussion} diskutiert die gewonnenen Erkenntnisse sowie die Grenzen und Einschränkungen der Ergebnisse kritisch und ordnet
sie gegenüber den Zielen und Fragestellungen der Arbeit ein.
Des Weiteren gibt das Kapitel einen Ausblick auf mögliche weitere Untersuchungen und Weiterentwicklungen des Prototyps.
