...
Neben den Identifikationsmerkmalen werden die eigentlichen Nutzdaten des Jobs als \gls{json}-Objekt persistent gespeichert.
...
Ein solches Szenario kann auftreten, wenn das Acknowledgment des Backends bei der Übertragung verloren geht und der Client
...
der dem Client mit dem \gls{http}-Statuscode \texttt{201 Created} als Bestätigung der erfolgreichen Joberstellung antwortet.
...
die Anfrage mit dem \gls{http}-Statuscode \texttt{200 OK}.
...
Erst wenn der Job erfolgreich gespeichert wurde oder bereits existiert, wird dem Client ein erfolgreicher \gls{http}-Statuscode zurückgegeben.
Kann der Job dagegen auch nach Ausschöpfen aller Wiederholungsversuche nicht gespeichert werden, antwortet das Backend
mit dem \gls{http}-Statuscode \texttt{500 Internal Server Error}.
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{llX}
        ...
        \midrule
        \gls{rest}-Schnittstelle & Spring Web & Bereitstellung und Nutzung der \gls{http}-Endpunkte und (De-)\allowbreak{}Serialisierung von \gls{json}\\
...
Für die Erstellung eines neuen Jobs erzeugt der Client eine \texttt{\gls{uuid}} und überträgt
diese zusammen mit den Nutzdaten des Jobs als \gls{dto} über die \gls{rest}-Schnittstelle an das Backend.
Dort nimmt der \texttt{JobController} das \gls{dto} entsprechend der Darstellung aus \cref{lst:endpoint} entgegen und validiert
...
Schlägt eine der beiden Validierungen fehl, beantwortet der Controller die Anfrage mit dem \gls{http}-Status \texttt{400 Bad Request}.
Anschließend delegiert der Controller die eigentliche Joberstellung an den \texttt{JobService}.
Nach erfolgreicher Erstellung gibt der Controller den \gls{http}-Status \texttt{201 Created} zurück.
Schlägt die Persistierung dagegen aufgrund einer Verletzung des \texttt{UNIQUE}-Constraints des als \texttt{\gls{uuid}}
realisierten Idempotenzschlüssels fehl, wird die resultierende \texttt{DataIntegrityViolationException} abgefangen.
...
mit dem \gls{http}-Status \texttt{200 OK} an den Client zurückgegeben.
...
Hierzu wird zunächst über eine Factory aus dem Request-\gls{dto} eine neue \texttt{Job}-Instanz erzeugt.
...
Netzwerkfehler, Verbindungsabbrüche sowie als temporär klassifizierte \gls{http}-Fehler berücksichtigt.
...
\begin{lstlisting}[
    ...
    Wiederholung einer Operation bei einem als transient klassifizierten Fehler.}},
