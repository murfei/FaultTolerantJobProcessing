%! Author = murfe
%! Date = 28.08.2026
%TODO: Schreibweise von Methodennamen in ganzer Arbeit nochmal prüfen: entweder einheutlich \texttt oder \textit
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/componentDiagram}%TODO: Schreibweise Recovery-Service in Abb. ohne -
...
\begin{figure}[h]
    ...
    zur Rückgabe einer Erfolgsnachricht an den Client inklusive Unterscheidung im Duplikatsfall des Idempotenz-Keys.}
...
\begin{figure}[h]
    ...
    ausgeführten Operation mit Seiteneffekten: Durch einen unklaren Verarbeitungszustand kann die Wiederaufnahme eines Jobs
    zu einer ungewollten Mehrfachausführung einer Operation führen.}
...
Datensätze die sich im übergebenen Zustand befinden zurück.
...
\begin{lstlisting}[
    ...
    ID. Dabei wird der zugehörigen Datensatzes für die Dauer der laufenden Transaktion mittels \textit{FOR UPDATE} gesperrt.}},
    label={lst:forUpdate}
]
SELECT *
FROM jobs
...
\begin{lstlisting}[
    ...
    Leases als Grundlage für die anschließende Recovery.}},
...
\begin{lstlisting}[
    language=Java,
    style=java,
    caption={[Übermittlung eines Jobs über die REST-Schnittstelle]{Übermittlung eines neuen Jobs vom Client an die
    \gls{rest}-Schnittstelle des Backends mittels HTTP-POST einschließlich der Nutzdaten und des Idempotency-Keys gekapselt
    durch ein \gls{dto}.}},
    label={lst:createJob}
]
...
\begin{lstlisting}[
    language=Java,
    style=java,
    caption={[\gls{rest}-Endpunkt zur Joberstellung]{\gls{rest}-Endpunkt zur Joberstellung mit Validierung der Anfrage, Delegation an
...
caption={[Generischer RetryExecutor]{Generische Steuerung von Wiederholungsversuchen: Ausführung einer Operation,
    Klassifikation auftretender Exceptions durch die Retry-Policy und ggfs. erneute Ausführung nach dem konfigurierten Backoff.}},
...
\end{lstlisting}
Die Datenbank-Policy klassifiziert ausschließlich Datenbankzugriffsfehler (\texttt{DataAccessException} und \texttt{CannotCreateTransactionException}),
...
Die Abstände zwischen den Recovery-Zyklen kann dabei über den Anwendungskontext konfiguriert werden.
