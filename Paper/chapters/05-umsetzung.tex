...
Daher ist die Spalte \textit{idempotency\_key} mit einem \texttt{UNIQUE}- und einem \texttt{NOT NULL}-Constraint belegt.
...
Wird beispielsweise wie in der schematische Darstellung aus \cref{fig:seiteneffekte} im Rahmen eines Jobs eine E-Mail versendet
...
Für die Erstellung eines neuen Jobs erzeugt der Client einen \gls{uuid}-basierten Itempotenzschlüssel und übertragt
...
Da die Verletzung des \texttt{UNIQUE}-Constraints nur einer Teilmenge der Exception ist, prüft der Controller auf den
Idempotenzfall.
...
Innerhalb der Methode wird für die Verarbeitung der Jobs aus dem Job-Store gesorgt, indem folgende Schritte solange wiederholt werden
...
wird der Aufruf von \textit{creatJob(request)} automatisch wiederholt.
