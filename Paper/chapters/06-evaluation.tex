...
Das Ziel dabei ist, die Parameter der unterschiedlichen Komponenten so zu wählen, dass praxisnahe Testbedingungen bei
gleichzeitig vertretbarem zeitlichem Aufwand ermöglicht werden.
...
\begin{longtable}{@{}p{0.045\textwidth}p{0.20\textwidth}p{0.33\textwidth}p{0.34\textwidth}@{}}
    ...
    \ref{zus:atomare_persistenz} & Ungültige Payload & Fachlich und syntaktisch invalide Requests senden & \texttt{\gls{http} 400}, kein DB-Eintrag; Zeilenzahl vor/nach unverändert \\
    \addlinespace
    \ref{zus:duplikaterkennung} & Acknowledgment geht verloren & Abfangen und verwerfen des Acknowledgments, stattdessen Fehler werfen & Client startet automatischen Retry. Die \gls{api} wird insgesamt 2 Mal angesprochen. Es wird genau ein Job im Backend gespeichert und der Statuscode der vom Client erhaltenen Antwort ist \texttt{200 OK} \\
    \addlinespace
...
Auf diese Weise wird das in \cref{subsec:atLeastOnce} beschriebene Szenario eines verlorenen Acknowledgments durch
...
Für Testfall (b) wird eine Anfrage mit einem bereits verwendeten Idempotenzschlüssel gesendet, um exemplarisch
die Behandlung eines als permanent klassifizierten Datenbankfehlers zu prüfen.
...
Beide Requests wurden vom Backend mit dem \gls{http}-Status \texttt{400 Bad Request} abgelehnt, der erste aufgrund der
...
korrekt als permanent eingestuft wurde.
Der zweite Client-Aufruf lieferte den \gls{http}-Status \texttt{200 OK} und denselben, bereits zuvor erzeugten Job zurück.
...
Der \texttt{RetryExecutor} klassifizierte den Fehler als transient und führte drei weitere Ausführungsversuche mit
Wartezeiten von 200\,ms, 400\,ms und 800\,ms durch.
Nach der Wiederherstellung der Datenbankverbindung schloss der vierte Versuch erfolgreich ab, sodass die Anfrage mit dem \gls{http}-Status
...
Die Anfrage wurde nach Erschöpfung aller konfigurierten Wiederholungsversuche mit dem \gls{http}-Status \texttt{500 Internal Server
...
Der \texttt{RetryExecutor} stufte diese als permanent ein, sodass kein weiterer Ausführungsversuch erfolgte.
...
Ein kurzzeitiger Datenbankausfall wurde korrekt als transient klassifiziert und mit exponentiell steigenden Wartezeiten
...
Ein permanenter Fehler wurde dagegen korrekt als nicht wiederholbar erkannt und ohne einen einzigen weiteren Ausführungsversuch
sofort beantwortet.
...
Insgesamt stützen die in \cref{sec:ergebnisse} dokumentierten Ergebnisse beide Hypothesen.
Kein durchgeführter Testfall lieferte einen Befund, der einer der Hypothesen widerspricht.
